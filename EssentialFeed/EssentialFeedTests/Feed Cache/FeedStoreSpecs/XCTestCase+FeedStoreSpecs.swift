...
extension FeedStoreSpecs where Self: XCTestCase {
    ...
    func assertThatRetrieveDeliversEmptyOnEmptyCache(on sut: FeedStore, file: StaticString = #file, line: UInt = #line) {
        //Retrieving empty cache
        expect(sut, toRetrieve: .empty, file: file, line: line)
    }

    func assertThatRetrieveHasNoSideEffectsOnEmptyCache(on sut: FeedStore, file: StaticString = #file, line: UInt = #line) {
        //Retrieving empty cache twice has no side effects
    ...
    func assertThatRetrieveHasNoSideEffectsOnNonEmptyCache(on sut: FeedStore,
                                                           file: StaticString = #file, line: UInt = #line) {
        //Retrieve inserted values twice has no side effects
        let feed = uniqueImageFeed().local
        let timestamp = Date()
    ...
    func assertThatInsertDeliversNoErrorOnEmptyCache(on sut: FeedStore, file: StaticString = #file, line: UInt = #line) {
        //Insert delivers no error on inserting to empty cache
    ...
    func assertThatInsertDeliversNoErrorOnNonEmptyCache(on sut: FeedStore, file: StaticString = #file, line: UInt = #line) {
        // Insert delivers no error on inserting to non-empty cache
    ...
    func assertThatInsertOverridesPreviouslyInsertedCacheValues(on sut: FeedStore, file: StaticString = #file, line: UInt = #line) {
        // Insert overrides previously inserted cache values
        insert((uniqueImageFeed().local, Date()), to: sut)
    ...
    func assertThatDeleteDeliversNoErrorOnEmptyCache(on sut: FeedStore, file: StaticString = #file, line: UInt = #line) {
        // Delete delivers no error on empty cache
    ...
    func assertThatDeleteHasNoSideEffectsOnEmptyCache(on sut: FeedStore, file: StaticString = #file, line: UInt = #line) {
        // Delete has no side effects on empty cache
    ...
    func assertThatDeleteDeliversNoErrorOnNonEmptyCache(on sut: FeedStore, file: StaticString = #file, line: UInt = #line) {
        //Delete delivers no error on non-empty cache
        insert((uniqueImageFeed().local, Date()), to: sut)
    ...
    func assertThatDeleteEmptiesPreviouslyInsertedCache(on sut: FeedStore, file: StaticString = #file, line: UInt = #line) {
        // Delete empties previously inserted cache
        insert((uniqueImageFeed().local, Date()), to: sut)
